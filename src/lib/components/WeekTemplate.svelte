<script lang="ts">
	...
	let { num, all_plans, settings } = $props();
...
<main class="flex justify-center">
	<h1
		class="w-25 sticky top-3 mb-16 mr-8 self-start text-2xl font-bold text-primary-foreground"
		...
		<NewDayTemplate day="Mon">
			{#each all_plans as plan}
				{#if plan.week_num === num && plan.day_of_week === 0}
					<Exercise {plan} {settings} day_done={() => day_done(plan.id)} />
				{/if}
			{/each}
		</NewDayTemplate>
